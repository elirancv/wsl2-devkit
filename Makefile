...
.PHONY: help verify verify-quiet lint demo bump-go checksums
...
checksums: ## Write checksums.txt (SHA256 of every script) for a release
	@sha256sum windows/*.ps1 wsl/*.sh | tee checksums.txt
	@echo "-> attach checksums.txt to the GitHub release (see CONTRIBUTING.md, Releasing)"

lint: ## Syntax-check + shellcheck the shell scripts (wsl/ + demo/lib/)
